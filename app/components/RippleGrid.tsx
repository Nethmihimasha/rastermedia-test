...
const RippleGrid: React.FC<Props> = ({
  ...
  useEffect(() => {
    ...
    // Helper to size canvas; if container is 0-sized during navigation, retry a few frames and then fallback to viewport
    const sizeCanvas = (attempt = 0) => {
      if (!containerRef.current) return;
      const bounds = containerRef.current.getBoundingClientRect();
      let width = Math.max(1, bounds.width);
      let height = Math.max(1, bounds.height);

      if ((bounds.width === 0 || bounds.height === 0) && attempt < 5) {
        requestAnimationFrame(() => sizeCanvas(attempt + 1));
        return;
      }

      // Fallback to viewport size if the container is still effectively 0 after retries
      if (width <= 1 || height <= 1) {
...
void main() {
    ...
    uniformsRef.current = uniforms; // ensure reference set before first render
    sizeCanvas();
    ...
    // --- RESIZE OBSERVER CHANGE ---
    const resizeObserver = new ResizeObserver((entries) => {
        for (const entry of entries) {
            // Using contentRect to get the precise size of the container
